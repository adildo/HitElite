      ...
      <div style={{ padding:'1.5rem 2rem' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1.25rem' }}>
          ...
          <div style={{ display:'flex', gap:'8px' }}>
            <div style={{ display:'flex', background:'#fff', border:'0.5px solid rgba(0,0,0,0.1)', borderRadius:'8px', padding:'3px' }}>
              {['list','sessions','cohorts'].map(function(v){ return <button key={v} onClick={function(){setView(v)}} style={{ ...btn, border:'none', background:view===v?'#f5f5f3':'transparent', fontWeight:view===v?600:400, padding:'5px 12px', fontSize:'12px' }}>{v.charAt(0).toUpperCase()+v.slice(1)}</button> })}
            </div>
        ...
        {view === 'cohorts' && (
          <div style={{ background:'#fff', border:'0.5px solid rgba(0,0,0,0.08)', borderRadius:'12px', padding:'2rem', textAlign:'center', color:'#888' }}>
            ...
            <div style={{ fontWeight:600, marginBottom:'6px', color:'#1a1a1a' }}>Recurring class enrollments</div>
            <div style={{ fontSize:'13px', marginBottom:'1.25rem' }}>Set up recurring class enrollments for students who attend the same class every week.</div>
            <a href="/admin/appointments?tab=recurring" style={{ padding:'10px 24px', background:'#D4A843', color:'#0D0D0D', borderRadius:'8px', fontSize:'13px', fontWeight:600, textDecoration:'none', display:'inline-block' }}>
...
// NOTE: recurring tab is handled in appointments.js
